...
select top 5 * from Employees order by FirstName
...
SELECT * FROM Employees WHERE Salary>=600 AND DepartmentName = 'HR'

SELECT * ISNULL noemail@example.com AS Email from Employees 
...
select coalesce (firstname, lastname) AS DisplayName from Employees
