...
Select e.Name,
d.DepartmentID from Employees as e 
...
SELECT c.FirstName,
o.orderID,
o.OrderDate from Customers as c 
join Orders as o
...
Select 
p. ProductName,
s.SupplierName from Products as p
Join Suppliers as s 
...
p.ProductName, 
s.SaleDate,
s.saleAmount from Products as P
join Sales as s
on s.ProductID = p.ProductID 
where s.SaleAmount > 400 and year(SaleDate) = 2022
...
sum (s.saleAmount*p.StockQuantity) as TotalSalesAmount  from Products as P
JOIN Sales AS s
...
join Departments as d 
on e.DepartmentID = d.DepartmentID
where d.DepartmentName = 'HR' and salary > 60000
...
SELECT p.ProductName, 
s.SaleDate,
p.StockQuantity  from Products as P
JOIN Sales AS s
on p.ProductID = s.ProductID
where p.StockQuantity > 100 and YEAR (SaleDate) = 2023
...
join Departments as d 
on e.DepartmentID = d.DepartmentID
where d.DepartmentName = 'sales' or  year(HireDate) > 2020
...
SELECT e.Name,
d.DepartmentName from Employees as e
join Departments as d 
on e.DepartmentID = d.DepartmentID
where d.DepartmentName in ('HR', 'Finance department') or e.Name like '%[a,i,e,o]%' 
...
join Departments as d 
on e.DepartmentID = d.DepartmentID
where d.DepartmentName in ('sales', 'marketing department') or e.Salary > 60000
