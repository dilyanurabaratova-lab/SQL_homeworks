...
select count(CustomerID) as cust_count from Customers

--Write a query to count the number of unique product categories from the Products table.

select count(distinct ProductName) as Proname_count from Products
...
select  ProductID, count(SaleAmount) as cnt_sale from Sales
where ProductID = '7'
group by ProductID
order by cnt_sale 
...
select  DepartmentName, count(EmployeeID) as ID_grade  from Employees
 where EmployeeID > 5
 group by DepartmentName 
 order by ID_grade
...
select  DepartmentName, count(EmployeeID) as CNT_ID  from Employees
...
SELECT OrderDate,  sum(TotalAmount) AS total_sales,
count(distinct ProductID) from orders
GROUP BY OrderDate
