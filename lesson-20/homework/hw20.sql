...
SELECT ParentId AS OlderPerson,
...
UPDATE #residents 
SET #residents .address = #residents .address + ' name=' + #residents .fullname
WHERE #residents .address NOT LIKE '%name=%';
...
(
    ...
    WHERE p.RoutePath NOT LIKE '%' + r.ArrivalCity + '%'   
)
SELECT TOP 1 'Cheapest Route' AS RouteType, RoutePath, Cost
FROM Paths
...
SELECT TOP 1 'Most Expensive Route' AS RouteType, RoutePath, Cost
FROM Paths
WHERE ArrivalCity = 'Khorezm'
ORDER BY Cost DESC;
...
SELECT GroupNum, STRING_AGG(Vals, ', ') AS ValuesInGroup
FROM Groups
GROUP BY GroupNum;
...
SELECT DISTINCT p.Name, p.Category, p.Price, p.Stock
FROM Products p
WHERE EXISTS (
    SELECT 1
    FROM Orders o
