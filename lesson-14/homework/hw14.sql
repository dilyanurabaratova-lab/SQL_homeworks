...
SELECT s.value AS Part, s.ordinal
FROM Splitter
CROSS APPLY STRING_SPLIT(Vals, '.', 1) s
...
WHERE DATEDIFF(YEAR, HIRE_DATE, GETDATE()) > 10
  ...
  --MEDIUM--
  --1write a SQL query to find all dates' Ids with higher temperature compared to its previous (yesterday's) dates.(weather)
...
SELECT 
    Vals,
    TRY_CAST(LEFT(Vals, PATINDEX('%[^0-9]%', Vals + 'X') - 1) AS INT) AS LeadingInteger
FROM GetIntegers;
...
SELECT 
    SUBSTRING(Vals, 2, 1) + 
    SUBSTRING(Vals, 1, 1) + 
    SUBSTRING(Vals, 3, LEN(Vals)-2) AS SwappedVals
FROM MultipleVals;
...
SELECT SUBSTRING('sdgfhsdgfhs@121313131', n, 1) AS ch
FROM cte;
...
JOIN (
 ...
 SELECT 
    TRANSLATE('rtcfvty34redt', '0123456789', '          ') AS chars,
    TRANSLATE('rtcfvty34redt', 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ', REPLICATE(' ',52)) AS nums;
