...
SELECT COALESCE(ssn, passportid, itin) AS first_non_null_value
FROM person
...
--2For every customer that had a delivery to California, provide a result set of the customer orders that were delivered to Texas. (Orders Table)
...
SELECT SequenceNumber,
       STRING_AGG(String, ', ') WITHIN GROUP (ORDER BY String) AS concatenated_values
FROM DMLTable
...
--4-The total number of employees in each department and the percentage of those employees who have been with the company for more than 3 years(Employees)

SELECT department_id,
       COUNT(*) AS total_employees,
       100.0 * SUM(CASE WHEN hire_date <= DATEADD(YEAR, -3, GETDATE()) THEN 1 ELSE 0 END) / COUNT(*) 
         AS pct_over_3_years
FROM Employees
